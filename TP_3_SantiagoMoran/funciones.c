...
//NO uso esta funcion pero la dejo para preguntar mas adelante algo
int agregarPelicula(sMovie* pMovie, sMovie* pAuxMovie, int movies, int* tam) {

    if (movies >= *tam - 1) {

        printf("\nLimite alcanzado. Se agregaran 5 lugares mas para proximas cargas.\n");

        pAuxMovie = (sMovie*)realloc(pMovie, (*tam + 2) * sizeof(sMovie));
...
void modificarPelicula(sMovie* pMovie, int movies) {
    ...
    if (index != -1) {
        ...
        fflush(stdin);
        gets(auxChar);
        opcion = getIntRange(auxChar, 1, 6);
